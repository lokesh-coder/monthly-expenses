...
class LocalDB {
  ...
  Future dumpData(List data) async {
  ...
  Future insertPayment(Payment payment) async {
    final Map paymentJson = payment.toJson();
  ...
  Future<List<Payment>> getAllPayments() async {
    ...
    return recordSnapshot
        .map((snapshot) => Payment.fromJson(snapshot.value))
        .toList();
  }

  Future<num> getDbVersion() async => (await db).version;
}
